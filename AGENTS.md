...
`docs/SHIPPING_OPERATIONS.md` records the authenticated US estimate and operating drill. Printful showed 9-11 business days for S, M, and L on 2026-07-12. Keep the two required delivery-range variables synchronized with current supplier evidence and Stripe's structured shipping estimate. This closes the catalogue-evidence gap only; customer-policy approval remains false until a Stripe test session visibly displays the range and a safe payment-to-fulfillment remedy drill passes.
...
The owner authenticated the existing Stripe `SiteQC` account on 2026-07-12. It is CAD-based and has prior live payout history, but Stripe still displays business/profile confirmation work. Protected Render now contains test-only payment and webhook credentials; checkout remains closed. Follow `docs/PAYMENT_OPERATIONS.md`. Never inspect a full API-key-page accessibility snapshot, pair test credentials with live mode, or treat dashboard access as PawSwipe activation or revenue.
