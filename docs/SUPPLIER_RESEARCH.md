...
- Printful file `1014307040` processed the public PNG successfully. Product template `104922382`, **PawSwipe Pet Parade Bandana**, was saved for S/M/L with a regular repeat at 30% scale and Printful's **Good / 152 DPI** quality result. A later line-editor audit proved the published sync variants use white stitching; the earlier black-stitching note was incorrect and has been corrected.
- A later authenticated dashboard audit on 2026-07-12 still found file `1014307040` in the File Library, but `Mes produits` showed no template card and a direct template-ID URL redirected to the empty template list. Reapplying the same file in a fresh editor produced a different default configuration at only **Okay / 46 DPI** before any save. That draft was discarded. Do not overwrite the evidenced 152 DPI template or call the lower-resolution editor default sample-ready; resolve the existing sync product through the provider API/order path first.
- The same live US destination view quoted **C$14.50** base cost, shipping from **C$5.59**, and **9-11 days** on 2026-07-12. This supersedes neither the protected USD supplier calculation nor the verified Stripe 9-11 business-day customer display; it is a current CAD dashboard observation that must be requoted at actual sample checkout.
...
- The template-only list can be empty while the store-specific `Publié` tab still contains sync product `445876313`. On 2026-07-12 that published row exposed all three exact external references. Opening M in the sample-order line editor proved regular repeat, 30% scale, **Good / 152 DPI**, and white stitching. Printful automatically saved a one-item sample-order draft at **C$11.60** after its 20% sample discount (C$14.50 original); shipping and tax remain unknown until a real delivery address is entered. No address, payment method, charge, submitted order, shipment, or sample approval exists.
